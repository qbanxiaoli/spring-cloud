...
public class StringUtil {
    ...
    public static String join(@SuppressWarnings("rawtypes") Iterator iterator, char separator) {
        ...
        StringBuilder builder = new StringBuilder(256); // Java默认值是16, 可能偏小
     ...
     * @return 如果找到，则返回<code>true</code>
     */
    private static boolean containsNone(String str, char[] invalid) {
     ...
     * @param stringCloses       characters which delimit the "close" of a string, in counterpart
     *                           order to <code>stringOpens</code>
        ...
        try {
            while ((currentChar = sourceReader.read()) != -1) {
                ...
                if (contextMarker == Character.MIN_VALUE && currentChar == '/'
                     ...
                     * else if (currentChar == '/' && slashSlashComments) { while ((currentChar =
                     * sourceReader.read()) != '\n' && currentChar != '\r' && currentChar >= 0) ; }
                     */
                } /*
                 * else if (contextMarker == Character.MIN_VALUE && currentChar == '#' &&
                 * hashComments) { // Slurp up everything until the newline while ((currentChar
                 * = sourceReader.read()) != '\n' && currentChar != '\r' && currentChar >= 0) ;
                 * }
                 */ else if (contextMarker == Character.MIN_VALUE && currentChar == '-'
                        && dashDashComments) {
    ...
    public static String replaceIgnoreCase(String s, String oldPattern, String newPattern) {
        return Pattern.compile(oldPattern, Pattern.CASE_INSENSITIVE).matcher(s)
                .replaceAll(newPattern);
    }

    /**
     * 验证输入的参数是否非空非""，如果为空则抛出InputStringIsNotValid异常，异常中使用errMsg作为参数。 如果非空则对其进行trim
     *
     * @param str    目标字段
